...
from transformers import AutoModel, AutoTokenizer, LlamaTokenizer, LlamaModel, AutoConfig
...
def parse_args():
    ...
    parser.add_argument('--domain', type=str, default='CDs_and_Vinyl')
    parser.add_argument('--max_his_len', type=int, default=50)
    parser.add_argument('--n_workers', type=int, default=16)
    parser.add_argument('--output_dir', type=str, default='ID_generation/preprocessing/processed/')
...
if __name__ == '__main__':
    ...
    if "blair" in args.plm:
        print("Using Blair's model")
        device = torch.device(args.device)
        tokenizer = AutoTokenizer.from_pretrained(args.plm)
        model = AutoModel.from_pretrained(args.plm).to(device)
        all_embeddings = []
        ...
        for pr in tqdm(range(0, len(sorted_text), args.batch_size)):
            batch = sorted_text[pr:pr + args.batch_size] if pr + args.batch_size < len(sorted_text) else sorted_text[pr:]
        ...
        embeddings_path = os.path.join(output_dir, f'{args.domain}.{args.plm.split("/")[-1]}.embeddings.npy')

    else:
        embeddings_path = os.path.join(output_dir, f'{args.domain}.embeddings.npy')
        text_embedding_model = SentenceTransformer('sentence-transformers/sentence-t5-base').to(device)
        all_embeddings = text_embedding_model.encode(sorted_text, show_progress_bar=True, batch_size=768 , convert_to_numpy=True)
