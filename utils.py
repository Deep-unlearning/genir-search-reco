...
from data import SeqRecDataset, ItemFeatDataset, ProductSearchDataset

def parse_global_args(parser):


    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--base_model", type=str, default="./ckpt/",help="basic model path")
...
def parse_dataset_args(parser):
    parser.add_argument("--data_path", type=str, default="./RQ-VAE/ID_generation/preprocessing/processed",
                        help="data directory")
    ...
    parser.add_argument("--index_file", type=str, default=".train.index.json", help="the item indices file")
    ...
    parser.add_argument("--add_prefix_his", action="store_true", default=False,
                        help="whether add prefix in history")
...
def parse_train_args(parser):
    ...
    parser.add_argument("--save_and_eval_steps", type=int, default=500)
...
def parse_test_args(parser):
    ...
    parser.add_argument("--metrics", type=str, default="hit@10,hit@50,ndcg@10,ndcg@50",
...
def load_datasets(args):
    ...
    for task, prompt_sample_num,data_sample_num in zip(tasks,train_prompt_sample_num,train_data_sample_num):
        if task.lower() == "seqrec":
            ...
            dataset = SeqRecDataset(args, mode="processed", mode_dataset=".train")
            # Print the longest sequence in the dataset
            # max_len = 0
            ...
            #     max_len = max(max_len, len(dataset[i]['input_ids']))
            # print(max_len)
        elif task.lower() == "productsearch":
            dataset = ProductSearchDataset(args, prompt_sample_num=prompt_sample_num, sample_num=data_sample_num)
        else:
    ...
    if "productsearch" in tasks:
        valid_data_ps = ProductSearchDataset(args, mode="test", sample_num=args.valid_prompt_sample_num)
        valid_data_seqrec = SeqRecDataset(args, mode="processed", mode_dataset=".valid")
        valid_data = ConcatDataset([valid_data_ps, valid_data_seqrec])
    else:
